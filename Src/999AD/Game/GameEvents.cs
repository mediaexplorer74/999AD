    ...
    private static void eventHandler(float elapsedTime)
    {
      switch (RoomsManager.CurrentRoom)
      {
        case RoomsManager.Rooms.tutorial0:
          ...
          if (FinalBoss.Dead && (double) Player.position.X < 200.0 
                        && (double) Player.position.Y <= (double) (176 - Player.height) 
                        && !GameEvents.eventAlreadyHappened[37])
          {
        ...
        case RoomsManager.Rooms.tutorial3:
          if (EnemyManager.enemyRoomManagers[(int) RoomsManager.CurrentRoom].enemiesType1[0].Dead
                        && !GameEvents.eventAlreadyHappened[2])
          {
          ...
          if (EnemyManager.enemyRoomManagers[(int) RoomsManager.CurrentRoom].enemiesType1[1].Dead
                        && !GameEvents.eventAlreadyHappened[4])
          {
            GameEvents.TriggerEvent(GameEvents.Events.keySpawns2_tutorial3);
            break;
          }
          if (!GameEvents.eventAlreadyHappened[4] || GameEvents.eventAlreadyHappened[5] 
                        || CollectablesManager.collectedItems.Count != 1)
            break;
        ...
        case RoomsManager.Rooms.churchBellTower0:
          if (Player.CollisionRectangle.Intersects(new Rectangle(196, 908, 96, 76))
                        && CollectablesManager.TryRemoveFromInventory(
                            Collectable.ItemType.doubleJump_powerup, new Vector2(223f, 938f))
                        && !GameEvents.eventAlreadyHappened[6])
          {
          ...
          if (Player.CollisionRectangle.Intersects(new Rectangle(196, 908, 96, 76)) 
                        && CollectablesManager.TryRemoveFromInventory(Collectable.ItemType.wallJump_powerup,
                        new Vector2(251f, 938f)) && !GameEvents.eventAlreadyHappened[8])
          {
        ...
        case RoomsManager.Rooms.midBoss:
          ...
          if (!GameEvents.eventAlreadyHappened[10] || CollectablesManager.collectedItems.Count <= 0 
                        || GameEvents.eventAlreadyHappened[11])
            break;
        ...
        case RoomsManager.Rooms.churchAltarRoom:
          ...
          if (!GameEvents.eventAlreadyHappened[16] || GameEvents.eventAlreadyHappened[17] 
        ...
        case RoomsManager.Rooms.church1stFloor0:
          if ((double) Player.position.X < 1032.0 && (double) Player.position.X >= 1000.0 
                        || (double) Player.position.X < 684.0 && (double) Player.position.X >= 664.0
                        || (double) Player.position.X < 364.0 && (double) Player.position.X >= 344.0 && (double) Player.position.Y > 106.0)
          {
        ...
        case RoomsManager.Rooms.escape0:
          ...
          if ((double) Player.position.X < (double) (MapsManager.maps[15].RoomWidthtPx - 248) 
                        && !GameEvents.eventAlreadyHappened[23])
            GameEvents.TriggerEvent(GameEvents.Events.lavaEruption1_escape0);

          if ((double) Player.position.X < (double) (MapsManager.maps[15].RoomWidthtPx - 416)
                        && !GameEvents.eventAlreadyHappened[24])
            GameEvents.TriggerEvent(GameEvents.Events.lowerFloor1_escape0);

          if (!PlatformsManager.platformsRoomManagers[15].movingPlatforms[1].active
                        && !GameEvents.eventAlreadyHappened[25] && GameEvents.eventAlreadyHappened[24])
          ...
          if ((double) Player.position.X < (double) (MapsManager.maps[15].RoomWidthtPx - 1352) 
                        && !GameEvents.eventAlreadyHappened[30])
            GameEvents.TriggerEvent(GameEvents.Events.lavaEruption5_escape0);
          if ((double) Player.position.X < (double) (MapsManager.maps[15].RoomWidthtPx - 1448)
                        && !GameEvents.eventAlreadyHappened[31])
        ...
        case RoomsManager.Rooms.escape1:
          ...
          if ((double) Player.position.X < (double) (MapsManager.maps[16].RoomWidthtPx - 881)
                        && !GameEvents.eventAlreadyHappened[34])
    ...
    public static void TriggerEvent(GameEvents.Events _event)
    {
      switch (RoomsManager.CurrentRoom)
      {
        case RoomsManager.Rooms.tutorial0:
          switch (_event)
          {
            ...
            case GameEvents.Events.finalCutscene:
              ...
              CutscenesManager.cutscenes[1].ChangeSentence(CutscenesManager.cutscenes[1].GetNUmOfSentences() - 1,
                  "Your Time: " + GameStats.GetTimeString() + "                                                                                            ");
        ...
        case RoomsManager.Rooms.tutorial3:
          switch (_event)
          {
            case GameEvents.Events.keySpawns1_tutorial3:
              CollectablesManager.collectablesRoomManagers[
                  (int) RoomsManager.CurrentRoom].AddCollectableToMap(
        ...
        case RoomsManager.Rooms.churchBellTower0:
          switch (_event)
          {
            case GameEvents.Events.unlockDoubleJump:
              ...
              AnimatedSpritesManager.animatedSpritesRoomManagers[
                  (int) RoomsManager.CurrentRoom].AddTempAnimatedSprite(
            ...
            case GameEvents.Events.unlockWallJump:
              ...
              AnimatedSpritesManager.animatedSpritesRoomManagers[
                  (int) RoomsManager.CurrentRoom].AddTempAnimatedSprite(
                  new AnimatedSprite(new Vector2(239f, 926f), AnimatedSprite.AnimationType.displayWallJumpRelic, false));
        ...
        case RoomsManager.Rooms.midBoss:
          ...
          CollectablesManager.collectablesRoomManagers[
              (int) RoomsManager.CurrentRoom].AddCollectableToMap(new Collectable(new Point(280, 140), 
              Collectable.ItemType.wallJump_powerup));

          CollectablesManager.collectablesRoomManagers[
              (int) RoomsManager.CurrentRoom].AddCollectableToMap(new Collectable(new Point(216, 148),
              Collectable.ItemType.brassKey));
          ...
          switch (_event)
          {
            case GameEvents.Events.showInvisibleTiles_church1stFloor0:
              ...
              for (int index1 = 0; index1 < MapsManager.maps[(int) RoomsManager.CurrentRoom].roomHeightTiles; ++index1)
              {
                ...
                {
                  if (MapsManager.maps[
                      (int) RoomsManager.CurrentRoom].array[index1, index2].tileType == Tile.TileType.solidEmpty)
                    animatedSprites.Add(
                        new AnimatedSprite(new Vector2((float) (index2 * Tile.tileSize),
                        (float) (index1 * Tile.tileSize)), AnimatedSprite.AnimationType.invisibleTile));
                }
              }
              AnimatedSpritesManager.animatedSpritesRoomManagers[
                  (int) RoomsManager.CurrentRoom].AddAnimatedSprites(animatedSprites);

              if ((double) Player.position.X < 1032.0 && (double) Player.position.X >= 1000.0)
                CameraManager.MoveCamera(0.0f, new Vector2(856f,
                    (float) MapsManager.maps[(int) RoomsManager.CurrentRoom].RoomHeightPx), 8f);
              else if ((double) Player.position.X < 684.0 && (double) Player.position.X >= 664.0)
                CameraManager.MoveCamera(0.0f, new Vector2(528f, 
                    (float) MapsManager.maps[(int) RoomsManager.CurrentRoom].RoomHeightPx), 8f);
              else if ((double) Player.position.X < 364.0 && (double) Player.position.X >= 344.0)
                CameraManager.MoveCamera(0.0f, new Vector2(204f,
                    (float) MapsManager.maps[(int) RoomsManager.CurrentRoom].RoomHeightPx), 8f);
        ...
        case RoomsManager.Rooms.finalBoss:
          switch (_event)
          {
            case GameEvents.Events.terrainCollapseFinalBoss:
              ...
              for (int index = 1; index < (MapsManager.maps[
                  (int) RoomsManager.CurrentRoom].roomWidthTiles + 1) / 2; ++index)
              {
